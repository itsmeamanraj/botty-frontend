    ...
    <section className="py-24 relative z-10 max-w-4xl mx-auto px-6">
      ...
      >
        {faqs.map((faq, idx) => (
          ...
          >
            <Accordion.Heading>
              <Accordion.Trigger className="w-full p-6 text-left flex items-center justify-between gap-4 text-white hover:text-accent transition-colors font-bold text-sm sm:text-base cursor-pointer">
                <span>{faq.q}</span>
                <Accordion.Indicator className="text-white font-bold w-8 h-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center flex-shrink-0" />
              </Accordion.Trigger>
            </Accordion.Heading>
            <Accordion.Panel>
              <Accordion.Body className="px-6 pb-6 text-xs sm:text-sm text-text-muted leading-relaxed border-t border-white/5 pt-4 font-normal">
                {faq.a}
              </Accordion.Body>
            </Accordion.Panel>
          </Accordion.Item>
